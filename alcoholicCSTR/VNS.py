...
for var_name in loadInfo['StartDirect'].keys():
...
GAMSfile = model+"_modelinstance_theta_nz"+str(Nz)+".gms"
...
deviated_t = gen_deviated_list(solver.time_len)
deviated_t = [1, 2, 4, 8, 16]
...
def shake(vertex, up_list,k):
    ...
    for up in up_list:
        ...
        dt = deviated_t[k]
        for vt in vertex[up] :
            pool.append([max(vt-dt, 1), vt, min(vt+dt, solver.time_len)])
        up_vertex = []
        for item in pool:
...
def LocalSearch(vertex, up_list, FirstMove=True):
    ...
    for up in up_list:
        pool[up] = []
        for vt in vertex[up] :
            pool[up].append([max(vt-1, 1), vt, min(vt+1, solver.time_len)])
...
def variable_neighborhood_search(vertex, kmax, uplist):
    ...
    while True:
        ...
        sol_tt, f_tt = LocalSearch(sol_t,uplist,FirstMove=False)
...
if __name__ == '__main__':
    ...
    saveFileName = os.getcwd()+'\\VNS-Nz'+str(Nz)+'-result'+str(datetime.now().strftime('%Y-%m-%d'))+'.pkl'
