...
for up in Uncertain_parameter_name:
    ...
    #nzLimitList[up] = other_restrict

solver = problem.AlcoholicCSTR()
solver.set_Nz(Nz)
...
class State(object):
    ...
    def is_terminal(self):
        ...
        num_limit = 0
        shift_Limit = nz*Nz+1
        #shift_Limit = other_restrict*len(Uncertain_parameter_name)
        for key in nz_list.keys():
            num_limit += nz_list[key]
    ...
    def set_available_switch_point(self):
        ...
        if is_finish == False:
            for up in Uncertain_parameter_name:
                ...
                if len(up_item['choices']) >= nzLimitList[up]:
                    continue
    ...
    def get_next_state_with_random_choice(self):
        ...
        act_list = []
        for item in self.cumulative_choices:
            if len(item['choices']) == nzLimitList[item['var']]:
                continue
...
def main():
    ...
    #init_state.set_cumulative_choices([{'var': 'Tout_x234', 'choices': [155]}, {'var': 'Tout_x567', 'choices': []}, {'var': 'Tout_x8910', 'choices': [155]}])
...
if __name__ == "__main__":
    ...
    if op_cmd == "MCTS" :
        ...
        import pickle
        saveFileName = os.getcwd()+'\\'+model+'MCTS-Nz'+str(Nz)+'-result'+str(datetime.now().strftime('%Y-%m-%d'))+'.pkl'
        ...
        test_ver = {'Tout_x2': [148, 195, 320], 'Tout_x34': [149, 195, 320], 'Tout_x567': [320], 'Tout_x89': [149, 195, 320], 'Tout_x10': [148, 195, 320]}
        ...
        for x1 in range(x-tau_k, x+tau_k+1):
            for x2 in range(x-tau_k, x+tau_k+1):
                for x3 in range(x-tau_k, x+tau_k+1):
                    test_ver = {'Tout_x2': [145, 191, x1], 'Tout_x34': [152, 189, x2], 'Tout_x567': [x3], 'Tout_x89': [152, 189, x2], 'Tout_x10': [145, 191, x1]}
