...
class SingleTank():
    def __init__(self,):
        ...
        self.model_name = "singletank"
        self.is_show_solve_result = True

        self.Nz = 1
        self.time_len = 800
        self.uncertainParaList = ["theta"]
        self.startDirect = {"theta": 1}
        self.GAMSModelInstance = None
        self.cp = None
        self.job = None
        self.cp_file = None

        # corresponding env act_name = {0:'+', 1:'-'}
        self.act_Table = {'theta' : {0: 1, 1:-1}}
...
if __name__ == "__main__":
    ...
    f = solver.solve({'theta': [110,370,620,'t']})
    #f = solver.solve_outDB({'theta': [180,540,'t']})
    """vertex = {'theta':[]}
    for t in range(800):
        #vertex['theta'].append(np.random.choice([0,1]))
        if t >= 320 :
            vertex['theta'].append(0)
        else:
            vertex['theta'].append(1)
    f = solver.solve_vertexList(vertex)"""
